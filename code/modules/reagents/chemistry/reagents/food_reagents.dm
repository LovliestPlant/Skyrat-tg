...
/datum/reagent/consumable/on_mob_life(mob/living/carbon/M, seconds_per_tick, times_fired)
	...
	if(ishuman(M))
		...
		if(!HAS_TRAIT(H, TRAIT_NOHUNGER))
			H.adjust_nutrition(nutriment_factor * REM * seconds_per_tick * 0.3) //NAAKASTATION EDIT: nutriment is only 30% as effective now to make up for the changes to metabolism - otherwise the dripfeed of nutrition would still overload you to omega chumby
